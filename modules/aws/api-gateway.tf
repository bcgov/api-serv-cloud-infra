...
##############APS Kong Gateway END#############################

##############APS Prometheus START#############################
resource "aws_apigatewayv2_api" "aps_prometheus_api" {
  name          = "APS Prometheus API"
  protocol_type = "HTTP"
  description   = "AWS HTTP API for Prometheus"
}

resource "aws_apigatewayv2_integration" "aps_prometheus_integration" {
  api_id           = aws_apigatewayv2_api.aps_prometheus_api.id
  integration_type = "HTTP_PROXY"

  integration_method = "ANY"
  integration_uri    = aws_lb_listener.http_listener_prom.arn
  connection_type    = "VPC_LINK"
  connection_id      = aws_apigatewayv2_vpc_link.aps_prometheus_vpc_link.id
}

resource "aws_apigatewayv2_route" "aps_prometheus_route" {
  api_id    = aws_apigatewayv2_api.aps_prometheus_api.id
  route_key = "GET /metrics"
  target    = "integrations/${aws_apigatewayv2_integration.aps_prometheus_integration.id}"
}

resource "aws_apigatewayv2_vpc_link" "aps_prometheus_vpc_link" {
  name               = "APS Prometheus VPC Link"
  security_group_ids = [aws_security_group.sg_ecs_adot_prom_collector.id]
  subnet_ids         = module.network.aws_subnet_ids.app.ids
}

resource "aws_apigatewayv2_stage" "aps_prometheus_stage_default" {
  api_id      = aws_apigatewayv2_api.aps_prometheus_api.id
  name        = "$default"
  auto_deploy = true
}
